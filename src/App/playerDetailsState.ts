...
const manualSeason$: Observable<SeasonDetails> = combineLatest({
  type: selectedType$,
...
export const selectedSeason$ = state<SeasonDetails | null>(
  currentSeasonIsActive$.pipe(
    filter((v) => v !== null),
    switchMap((isActive) => (isActive ? currentSeason$ : manualSeason$))
  ),
  null
);
